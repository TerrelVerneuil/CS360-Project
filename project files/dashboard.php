...
//        echo("<DIV class='main-content'><H1>" .$_GET['IID']. "$uid</H1>");
    ...
    echo("<H1 style='margin-top:0'>Picked for you</H1>");
    if(isset($_SESSION['uid'])){
        ...
        if($randomItem != FALSE){
            for($x = 0; $x < 10; $x++){
                ...
                echo("<A style='font-size: 20px; font-weight: 600' href=?op=displayItem&IID=".$row['itemID'].">".$row['name']."</A>");
                echo("<P style='margin-top:0'>".$row['description']."</P>");
...
?>
<DIV class="sidenav">
<A href="?op=home"><p style='font-size: 32px; margin-top: 1rem; margin-bottom: 0'>MARKETPLACE</p></A>
...
if (isset($_SESSION['uid'])) {
?>
	<a href="?op=showAccountManagement" style='margin-left:1rem; color: #4F1F33'><?php echo ("👤 " . htmlspecialchars($uname)) ?></a>
<?php
    showLogoutForm();
}
else {
    ?>
    <A id="username" href="?op=showLoginForm" style='margin-left: 1rem;'>👤 login</A>
...
<DIV class="boxcontainer">
    <DIV class="box"><A href="?op=cart"><i style="font-size:24px; padding-right: 0.5rem" class="fa">&#xf07a;</i></A></DIV>
...
<FORM name='filterResults' method='POST' action='?op=searchItems' style='margin-left: 2rem'>
<INPUT id="searchbar" name = "searchbar" type="text" placeholder="Search...">    <!--will clean this up a bit later -->
<A style='color: #4F1F33' href=<?php if($op != 'filter'){echo("?op=filter");} else{echo("dashboard.php");} ?>>filter</A>
