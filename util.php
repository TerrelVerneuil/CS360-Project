	...
	if($res != FALSE && $res-> rowCount() == 1){
		...
		if($curr_password === $current_password){
			if($newPassword !== $curr_password&&$newPassword ===$confirmPassword){
				echo "<br><p class='warning-style'>Successfully Changed Password </p>";
				showAccountManagementForm($db, $uid);
				
				return true;
			}else{
				echo "<br><p class='warning-style'>New Password & Confirm New Password fields must match </p>";
				
				showAccountManagementForm($db, $uid);
				return false;
			}
		}else{
			echo "<br> <p class='warning-style'>invalid current password </p>";
			showAccountManagementForm($db, $uid);
...
function displayItem($db, $iid){
  ...
  if($res != FALSE){
    ...
    <FORM name='addToWishlist' method='get'>
	<input type='hidden' name='op' value = 'wishlist' />
	<INPUT TYPE='hidden' name='IID' value='<?php echo($iid); ?>' />    
...
function addToWishlist($db, $iid, $uid){
  $res = $db->query("SELECT * FROM item WHERE itemID = $iid");
  ...
  if($res != FALSE){
    ...
    $sellerID = $item['sid'];
    if($sellerID == $uid){
      cannotAddToCart("You cannot place items you sell on your wishlist");
    }
    else{
      $res3 = $db->query("INSERT INTO wishes_for(itemID, userID) VALUES ($iid, $uid)");
      if($res3 != FALSE){
        ?>
        <DIV class='main-content'>
...
function showWishlist($db, $uid){
  $res = $db->query("SELECT * FROM wishes_for NATURAL JOIN item WHERE userID = $uid");
  ...
  if($res != FALSE){
    ...
    $res3 = $db -> query("SELECT SUM(price) AS total FROM wishes_for NATURAL JOIN item WHERE userID = $uid");
    if($res3 != FALSE){
      $price = $res3->fetch();
      echo("<DIV class='main-content'>");
