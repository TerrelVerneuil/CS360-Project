...
//        echo("<DIV class='main-content'><H1>" .$_GET['IID']. "$uid</H1>");
	...
	if(!isset($_SESSION['uid'])){
		...
		addToWishlist($db, $_GET['IID'], $uid);
	}

}

else if($op == "wishlist_show"){
	if(!isset($_SESSION['uid'])){
		handleLoginForm($db, $op);
		exit;
	}else{
		...
		$db->query("DELETE FROM wishes_for WHERE itemID=$removeItem AND userID=$uid");	
		}	
		showWishlist($db,$uid);
...
<DIV class="boxcontainer">
    <DIV class="box"><a href="?op=wishlist_show"><i style="font-size:24px" class="fa">&#xf004;</i></a></DIV>
    <DIV class="text-under-box"></DIV>
